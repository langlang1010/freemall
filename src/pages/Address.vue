        ...
        <div class="container">
          <div class="checkout-addr">
              ...
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" 
                      v-bind:class="{'check':checkedIndex==index}" @click="checkedIndex=index;"
                      :key="index">
                    <dl>
                      <dt>{{item.userName}}</dt>
            ...
            <div class="next-btn-wrap">
              <a class="btn btn--m btn--red" href="#" @click="next()">下一步</a>
            </div>
          </div>
        </div>
      </div>
      <Modal :mdShow="modalShow" @close="modalShow=false">
        <p slot="message">更多实战讲解，请前往学习新课《Vue全家桶从0打造小米商城》</p>
        <a slot="btnGroup" class="btn btn--m btn--red" href="javascript:;" @click="modalShow = false">关闭</a>
      </Modal>
...
<script>
  ...
  export default{
      data(){
          return{
            limit:3,//默认限制3个
            checkedIndex:0,//当前选中的索引
            addressList:[],//地址列表
            modalShow:false//下一步、弹框提示
          }
      },
      components:{
        NavHeader,
